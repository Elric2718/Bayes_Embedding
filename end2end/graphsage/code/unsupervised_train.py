...
from graphsage.models import SampleAndAggregate, SAGEInfo, Node2VecModel, BemNode2VecModel
...
flags.DEFINE_string('kg_path', '', "The path of KG.")
...
def train(train_data, test_data=None):
    ...
    if FLAGS.model == 'graphsage_mean':
        ...
        model = Node2VecModel(placeholders, features.shape[0],
                                       ...
                                       lr=FLAGS.learning_rate)
    ### bem part ###    
    elif FLAGS.model == 'bem_n2v':
        #n_w = 100        
        #w_embeds = np.random.normal(0., 1., size = [features.shape[0], n_w])
        # w_embeds = pd.read_csv(FLAGS.kg_path, sep = "#", names = ["item_id", "data"],  usecols = ["data"], skiprows = 1).values
        # w_embeds = np.array([line[0].split(',') for line in w_embeds]).astype(float)
        w_embeds = np.load(FLAGS.train_prefix + '-kgfeats.npy')
        n_w = w_embeds.shape[1]
        print(features.shape[0], w_embeds.shape[0])
        ...
        model = BemNode2VecModel(placeholders,\
                                features.shape[0],\
                                minibatch.deg,\
                                #2x because graphsage uses concat
                                nodevec_dim=2*FLAGS.dim_1,\
    ...
    if FLAGS.save_embeddings:
        ...
        if FLAGS.model == "n2v" or FLAGS.model == "bem_n2v":
            ...
            start_time = time.time()
            print("Doing test training for n2v/bem_n2v.")
            test_steps = 0
