...
import graphsage.bem_e2e as bem
...
class BemNode2VecModel(GeneralizedModel):
    def __init__(self, placeholders,\
        ...
        super(BemNode2VecModel, self).__init__(**kwargs)
        ...
        ### bem part ###
        self.bem_n_w = w_embeds.shape[1]
        self.bem_n_z = nodevec_dim
        self.bem_n_hidden = bem_n_hidden
        ...
        self.w_embeds = tf.constant(w_embeds, dtype=tf.float32) # in kwargs
        ### bem part ###
    ...
    def _build(self):
        ...
        self.outputs1 = tf.reshape(tf.nn.embedding_lookup(self.target_embeds, self.inputs1), [-1, self.bem_n_z])
        self.outputs2 = tf.nn.embedding_lookup(self.context_embeds, self.inputs2)
        self.outputs2_bias = tf.nn.embedding_lookup(self.context_bias, self.inputs2)
        self.neg_outputs = tf.nn.embedding_lookup(self.context_embeds, self.neg_samples)
        self.neg_outputs_bias = tf.nn.embedding_lookup(self.context_bias, self.neg_samples)
        ...
        ### bem part ###        
        w1_inputs = tf.reshape(tf.nn.embedding_lookup(self.w_embeds, self.inputs1, partition_strategy = "div"), [-1, self.bem_n_w])
        w2_inputs = tf.concat([tf.nn.embedding_lookup(self.w_embeds, self.inputs2), tf.nn.embedding_lookup(self.w_embeds, self.neg_samples)], axis = 0)
        z1_inputs = self.outputs1
        z2_inputs = tf.concat([self.outputs2, self.neg_outputs], axis = 0)
        mapping_index_pos = tf.concat([tf.reshape(tf.range(self.batch_size), [self.batch_size, 1]), tf.reshape(tf.range(self.batch_size), [self.batch_size, 1])], axis = 1)
        mapping_index_neg = tf.concat([tf.reshape(tf.tile(tf.range(self.batch_size), [FLAGS.neg_sample_size]), [self.batch_size * FLAGS.neg_sample_size, 1]),\
                                           tf.reshape(tf.tile(tf.expand_dims(tf.range(self.batch_size, self.batch_size + FLAGS.neg_sample_size), -1),  [1, self.batch_size]), [FLAGS.neg_sample_size * self.batch_size, 1])], axis = 1)
        mapping_index = tf.concat([mapping_index_pos, mapping_index_neg], axis = 0)

        #print(w1_inputs, w2_inputs, z1_inputs, z2_inputs, mapping_index, tf.concat([z1_inputs, w1_inputs], axis = 1))
        self.bem_model = bem.BayesNetwork(n_batch = tf.shape(mapping_index)[0],\
                            n_w = self.bem_n_w,\
                            n_hidden = self.bem_n_hidden,\
                            n_z = self.bem_n_z,\
                            w1_inputs = w1_inputs,\
                            w2_inputs = w2_inputs,\
                            z1_inputs = z1_inputs,\
                            ...
                            nf_K = self.nf_K,\
                            pair_or_single = self.pair_or_single)#,\
                            #seed = self.seed,\
    ...
    def _loss(self):
        ...
        ### bem part ###
        bem_loss = self.bem_model._create_model()[0]        
        ### bem part ###


        
        loss = tf.reduce_sum(true_xent) + tf.reduce_sum(negative_xent) + bem_loss
        self.loss = loss / tf.cast(self.batch_size, tf.float32)
        tf.summary.scalar('loss', self.loss)
